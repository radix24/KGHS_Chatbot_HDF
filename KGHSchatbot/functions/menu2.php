...
function getmeal2($days)
{
  ...
  if ($date == "2018.07.03"){
   $final = "오늘은 기말고사 첫째날이에요! 급식은 없지만 영양보충해서 힘내고 다들 시험 화이팅 :)";
   $return = array($date, $final); // 해당날짜, 급식메뉴
   return $return;
 }
 if ($date == "2018.07.04"){
  $final = "오늘은 기말고사 둘째날이에요! 급식은 없지만 영양보충해서 힘내고 다들 시험 화이팅 :)";
  $return = array($date, $final); // 해당날짜, 급식메뉴
  return $return;
}
if ($date == "2018.07.05"){
 $final = "오늘은 기말고사 셋째날이에요! 급식은 없지만 영양보충해서 힘내고 다들 시험 화이팅 :)";
 $return = array($date, $final); // 해당날짜, 급식메뉴
 return $return;
}
if ($date == "2018.07.06"){
 $final = "오늘은 기말고사 넷째날이에요! 급식은 없지만 영양보충해서 힘내고 다들 시험 화이팅 :)";
 $return = array($date, $final); // 해당날짜, 급식메뉴
 return $return;
}
if ($date == "2018.07.07"){
 $final = "오늘은 기말고사 기간이에요! 4일차 시험까지 열심히 달려왔으니 토요일에는 쉬어가면서 다음주 시험을 위한 체력보충! 급식은 없지만 다들 시험 화이팅 :)";
 $return = array($date, $final); // 해당날짜, 급식메뉴
 return $return;
}
if ($date == "2018.07.08"){
 $final = "오늘은 기말고사 기간이에요! 급식은 없지만 내일 마지막 시험도 다들 화이팅 :)";
 $return = array($date, $final); // 해당날짜, 급식메뉴
 return $return;
}
if ($date == "2018.07.09"){
 $final = "오늘은 기말고사 다섯째날이에요! 급식은 없지만 영양보충해서 힘내고 다들 시험 화이팅 :)";
 $return = array($date, $final); // 해당날짜, 급식메뉴
 return $return;
}
